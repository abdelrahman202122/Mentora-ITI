...
export default function EditProfileForm({ tutorId }: { tutorId: string }) {
  ...
  if (isProfileLoading) return <p>{t('loading')}</p>;
  if (profileError || !tutorProfile) return <p>{t('error')}</p>;
  const { headline, bio, hourlyRate } = tutorProfile;
  const subjects = tutorSubjects || [];
  const rateId = `${tutorId}-hourly-rate`;
    ...
    <div className="min-h-screen bg-background text-foreground">
      <div className="max-w-5xl mx-auto px-6 py-10 space-y-10">
        ...
        <div className="flex flex-col md:flex-row justify-between gap-6">
          ...
          <div className="flex gap-3">
            <button className="btn-outline">{t('discardChanges')}</button>
        ...
        <section className="space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            >
              ...
              <div className="text-center">
                <p className="font-bold">{t('addNewCourse')}</p>
                <p className="text-sm text-muted-foreground">
